...
public class ShopMakeOrderActivity extends BaseActivity<MVPPresenter, MVPModel> implements MVPContract.View {
    ...
    public static String KEY_SERVICE = "SERVICE_NAME";
    ...
    public static String KEY_MARKET = "MARKET";
    ...
    protected int getLayoutResId() {
        return R.layout.activity_shop_makeorder;
    }
    ...
    protected void bindEvent() {
        ...
        tvPay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                String time = TimeUtils.formateTime(startTime,TimeUtils.FROMATE_YMD)+","+TimeUtils.formateTime(endTime,TimeUtils.FROMATE_YMD);
...
//                prePayIdAsyncTask.execute(urlString,UIUtil.getUUID());
    ...
    protected void bindData() {
        ...
        serviceName = getIntent().getExtras().getString(KEY_SERVICE);
        money = getIntent().getExtras().getString(KEY_MARKET);
        startTime = getIntent().getExtras().getLong(KEY_START_TIME);
        endTime = getIntent().getExtras().getLong(KEY_END_TIME);
        market = getIntent().getExtras().getString(KEY_MARKET);
        coupon_id = getIntent().getExtras().getString(KEY_COUPON);
        num = getIntent().getExtras().getInt(KEY_NUM);

        tvShopname.setText(dataBean.getStore_name());
        tvService.setText(serviceName);
        tvTotalMoney.setText("￥"+(dataBean.getPrice()*num));
        tvPayMoney.setText("￥"+dataBean.getPrice()*num);
        tvServiceTime.setText(TimeUtils.formateTime(startTime,TimeUtils.FROMATE_YMHM_ZH)+"-"+TimeUtils.formateTime(endTime,TimeUtils.FROMATE_YMHM_ZH));
        tvMarket.setText(market);
        tvPay.setText("确认支付：￥"+dataBean.getPrice()*num);
    }
    ...
    private void  requestAlipay(String orderId){
        ...
        String subject=dataBean.getStore_name();
        if(!TextUtils.isEmpty(coupon_id))
        {
            subject=subject+"+"+coupon_id;
        }
        Map<String, String> params = OrderInfoUtil2_0.buildOrderParamMap(Constants.APPID,rsa2,dataBean.getPrice()*num,"订单支付",orderId,Constants.NOTIFY_URL,subject);
    ...
    private String getProductArgs(String orderId) {
        ...
        try {
            ...
            packageParams.add(new BasicNameValuePair("body", getString(R.string.app_name)+"-"+getString(R.string.recharge)));
            ...
            int money =(int)(dataBean.getPrice()*100*num);
