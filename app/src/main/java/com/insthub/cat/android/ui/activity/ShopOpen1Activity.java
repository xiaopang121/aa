...
public class ShopOpen1Activity extends BaseActivity<MVPPresenter, MVPModel> implements MVPContract.View {
    ...
    protected void bindEvent() {
        ...
        btPublish.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                {
                    ToastUtil.show(getContext(),"请上传店铺详情");
                    return ;
                }
                ...
                if(bannerImages.size()>0)
                {
                    ...
                    {
                        sbBanner.append(!TextUtils.isEmpty(bannerImages.get(x).getSave_path()) ? bannerImages.get(x).getSave_path(): bannerImages.get(x).getFull_path() );
                ...
                KLog.i("sbBanner:"+sbBanner.toString());
                ...
                if(exImageList.size()>0)
                {
                    ...
                    {
                        sbDetiaoImage.append(!TextUtils.isEmpty(exImageList.get(x).getSave_path()) ? exImageList.get(x).getSave_path(): exImageList.get(x).getFull_path());
                ...
                KLog.i("detialBanner:"+sbDetiaoImage.toString());




                showLoadDialog("正在更新数据");
                mPresenter.updateStore(CacheManager.getInstance().getToken().getData().getUser_id(),
                        ...
                        tvServiceType.getText().toString(),
                        etShopAddress.getText().toString(),
                        etShopGoodness.getText().toString(),
                        sbDetiaoImage.toString(),
                        logo==null ? dataBean.getLogo():logo.getData().getSave_path(),
                        BDLocationManager.getInstance().getCurLocation().getLongitude(),
                        BDLocationManager.getInstance().getCurLocation().getLatitude(),
                        tagList==null ? dataBean.getLabel_lv1():String.valueOf(tagList.getLabel_id()),
                        childTags==null ? dataBean.getLabel_lv2():childTags,
    ...
    protected void bindData() {
        ...
        if(dataBean!=null)
        {
            ...
            etShopName.setText(dataBean.getStore_name());

            tvServiceType.setText(dataBean.getService_content());


            etServicePrice.setText(dataBean.getService_price());


            etShopGoodness.setText(dataBean.getDetail());


            etShopAddress.setText(dataBean.getAddress());

            etShopContact.setText(dataBean.getContacts());

            etShopPhone.setText(dataBean.getPhone());


            etShopBankname.setText(dataBean.getBank());

            etShopBankcard.setText(dataBean.getBank_account());

            bannerImages.clear();

            for(int x=0;x<dataBean.getBanner_list().size();x++)
            {
                bannerImages.add(new UploadImageInfoData.DataBean(dataBean.getBanner_list().get(x)));
                RequestOptions requestOptions4 = new RequestOptions()
                        .centerCrop()
                        .placeholder(R.drawable.ic_add_grey_600)
                        .error(R.drawable.ic_add_grey_600);
                Glide.with(getContext()).asBitmap()
                        .load(dataBean.getBanner_list().get(x))
            ...
            for(int x=0;x<dataBean.getImage_list().size();x++)
            {
                exImageList.add(new UploadImageInfoData.DataBean(dataBean.getImage_list().get(x)));
                RequestOptions requestOptions4 = new RequestOptions()
                        .centerCrop()
                        .placeholder(R.drawable.ic_add_grey_600)
                        .error(R.drawable.ic_add_grey_600);
                Glide.with(getContext()).asBitmap()
                        .load(dataBean.getImage_list().get(x))
                        .apply(requestOptions4)
