...
public class DBManager {
    ...
    public List<City> searchDistarct(final String keyword){
        SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(DB_PATH + DB_NAME, null);
    ...
    public List<City> searchProvicne(){
        SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(DB_PATH + DB_NAME, null);
        Cursor cursor = db.rawQuery("select * from " + TABLE_NAME +" group by province  order by _id", null);
        List<City> result = new ArrayList<>();
        City city;
        while (cursor.moveToNext()){
    ...
    public List<City> searchCitysByProvicne(String provicneName){
        SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(DB_PATH + DB_NAME, null);
        Cursor cursor = db.rawQuery("select * from " + TABLE_NAME +" where province = '"+provicneName+"'  order by _id", null);
        List<City> result = new ArrayList<>();
        City city;
        while (cursor.moveToNext()){
    ...
    public List<City> searchDistrictByCity(String cityname){
        SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(DB_PATH + DB_NAME, null);
        Cursor cursor = db.rawQuery("select * from " + TABLE_NAME +" where city = '"+cityname+"'  order by _id", null);
        List<City> result = new ArrayList<>();
        City city;
        while (cursor.moveToNext()){
            String name = cursor.getString(cursor.getColumnIndex("name"));
