...
public class CircleProgress extends View {
    ...
    private int[] mGradientColors = {getResources().getColor(R.color.controller_value_color),getResources().getColor(R.color.controller_value_color),getResources().getColor(R.color.controller_value_color)};
    //当前进度，[0.0f,1.0f]
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mValueColor = typedArray.getColor(R.styleable.CircleProgressBar_valueColor, getResources().getColor(R.color.controller_value_color));
        ...
        mUnitColor = typedArray.getColor(R.styleable.CircleProgressBar_unitColor, getResources().getColor(R.color.B_black_50));
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //求最小值作为实际值
        int minSize =getHeight()- getPaddingTop()*4 - getPaddingBottom() - 2 * (int) maxArcWidth;
        //减去圆弧的宽度，否则会造成部分圆弧绘制在外围
        mRadius = minSize*1.1f;
        //获取圆的相关参数
        mCenterPoint.x = w / 2;
        mCenterPoint.y = h-getPaddingBottom()*4;
        //绘制圆弧的边界
        ...
        mValueOffset = mCenterPoint.y - getBaselineOffsetFromY(mValuePaint)-20;
        mHintOffset = mCenterPoint.y - mRadius * mTextOffsetPercentInRadius + getBaselineOffsetFromY(mHintPaint);
        mUnitOffset = mCenterPoint.y  + getBaselineOffsetFromY(mUnitPaint)-10;
        updateArcPaint();
    ...
    private void drawText(Canvas canvas) {
        ...
        {
            canvas.drawText(String.format(mPrecisionFormat, tempview), mCenterPoint.x, mValueOffset, mValuePaint);
        }
