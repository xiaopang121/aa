...
import com.insthub.cat.android.ui.dialog.ShowOtherPintuanDialogFragment;
...
public class PingtuanDetialActivity extends BaseActivity<MVPPresenter, MVPModel> implements MVPContract.View, LocalImgAdapter.BannerCallback {
    ...
    @Bind(R.id.tv_pinp_list)
    TextView tvPinTitle;


    @Bind(R.id.iv_pintuan_head)
    RoundedImageView  tvPintuanHead;

    @Bind(R.id.tv_pintuan_name)
    TextView tvPintuanName;

    @Bind(R.id.tv_pingtuan_last_people)
    TextView tvPintuanLastPeople;


    @Bind(R.id.tv_pingtuan_last_time)
    TextView tvPintuanLastTime;


    @Bind(R.id.tv_pintuan_do)
    TextView tvPintuanDo;

    @Bind(R.id.rll_pinp_more)
    RelativeLayout rllPinMore;
    ...
    private PintuanDetialData mStoreDetialData;
    ...
    protected int getLayoutResId() {
        return R.layout.activity_pintuan_detial;
    }
...
//        mLBanners.setHoriZontalCustomTransformer(new ParallaxTransformer(R.id.id_image));//自定义样式
    ...
    private void updateView() {
        ...
        if (mStoreDetialData != null) {
            ...
            tvBuyPrice.setText("¥" + mStoreDetialData.getData().getActivity().getDiscount_price());
            ...
            if(mStoreDetialData.getData().getImage_list().size()>0)
            {
             ...
             tvPinTitle.setText(mStoreDetialData.getData().getPintuan_count()+"人在团购");


            Glide.with(getContext())
                    .load(mStoreDetialData.getData().getPintuan().getHead_image())
                    .into(tvPintuanHead);



            tvPintuanName.setText(DeclareUtil.formatePhone(mStoreDetialData.getData().getPintuan().getPhone()));
            ...
            tvPintuanLastPeople.setText(spannableString);



            if(!TextUtils.isEmpty(mStoreDetialData.getData().getPintuan().getPhone()) && !TextUtils.isEmpty(mStoreDetialData.getData().getPintuan().getCreate_time()) )
            {
                long endTime =  TimeUtils.parserTime(mStoreDetialData.getData().getPintuan().getCreate_time(),TimeUtils.FROMATE_YMD);
                ...
                mHelper = new CountTimeDownHelper(new CountDownTimer(distancTime,1 * 1000 - 10) {
                    @Override
                    public void onTick(long millisUntilFinished) {
                        String counteTime = TimeUtils.formateTime(millisUntilFinished,TimeUtils.FROMATE_HMS);
                        tvPintuanLastTime.setText("剩余 "+counteTime);
                    }

                    @Override
                    public void onFinish() {
                        tvPintuanLastTime.setText("剩余 00:00:00");
                    }
                });
                mHelper.start();
            }
    ...
    protected void bindEvent() {
        ...
        llyBuy.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(CacheManager.getInstance().getToken()==null)
                {
                ...
                showLoadDialog("正在发起团购");

                mPresenter.createPintuan(CacheManager.getInstance().getToken().getData().getUser_id(),
                        CacheManager.getInstance().getToken().getData().getToken(),
                        String.valueOf( mStoreDetialData.getData().getActivity().getActivity_id()),
                        mStoreDetialData.getData().getActivity().getRemarks());
        ...
        rllPinMore.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Bundle bundle = new Bundle();
                bundle.putInt(ConstantsKeys.KEY_DATA,mStoreDetialData.getData().getActivity().getActivity_id());
               // startActivity(PintuanListActivity.class,bundle);
                if(mStoreDetialData==null)
                {
                ...
                ShowOtherPintuanDialogFragment mm = new ShowOtherPintuanDialogFragment();
                mm.setArguments(bundle);
                mm.setDismissCallback(new ShowOtherPintuanDialogFragment.DismissCallback() {
                    @Override
                    public void dismiss() {
                        DialogUtils.showPinTuanDialog(getActivity(), mStoreDetialData, new PintuanCallback() {
                            @Override
                            public void onClickSure() {



                                showLoadDialog("正在参与拼团");

                                mPresenter.joinPintuan(CacheManager.getInstance().getToken().getData().getUser_id(),
                                        CacheManager.getInstance().getToken().getData().getToken(),
                                        mStoreDetialData.getData().getPintuan().getActivity_id(),
                                        mStoreDetialData.getData().getPintuan().getPintuan_id(),
                                        mStoreDetialData.getData().getPintuan().getLeader());

                            }
                        }).show();
                    }
                });
                mm.show(getSupportFragmentManager(),"12321321");
        ...
        tvPintuanDo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        tvShare.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                mShareListener = new CustomShareListener(PingtuanDetialActivity.this);
    ...
    protected void bindData() {
        ...
        mPresenter.getPintuanDetail(String.valueOf(data.getActivity_id()));
    ...
    public void showSuccess(Object object) {
        ...
        dismissLoadDialog();
        if (object instanceof PintuanDetialData) {
            mStoreDetialData = (PintuanDetialData) object;
            updateView();
...
//                             .into(ivCommentHead);
        ...
        if(object instanceof JoinPintuanData)
        {
            Bundle bundle = new Bundle();
            bundle.putSerializable(ConstantsKeys.KEY_DATA,mStoreDetialData);
            startActivity(PintuanMakeOrderActivity.class,bundle);
        }



        if(object instanceof CreatePintuanData)
        {
            Bundle bundle = new Bundle();
            bundle.putSerializable(ConstantsKeys.KEY_DATA,mStoreDetialData);
            startActivity(PintuanMakeOrderActivity.class,bundle);
    ...
    private static class CustomShareListener implements UMShareListener {
        ...
        private WeakReference<PingtuanDetialActivity> mActivity;

        private CustomShareListener(PingtuanDetialActivity activity) {
