 ...
 */
package com.common.android.fdao.utils;
...
import com.common.android.fdao.orm.*;
...
public class AbDBDaoImpl<T> extends AbBasicDBDao implements AbDBDao<T> {
	...
	public AbDBDaoImpl(SQLiteOpenHelper dbHelper, Class<T> clazz) {
		...
		KLog.d(AbDBDaoImpl.class, "clazz:" + this.clazz + " tableName:" + this.tableName
	...
	public T queryOne(int id) {
		synchronized (lock) {
			...
			KLog.d(AbDBDaoImpl.class, "[queryOne]: select * from " + this.tableName + " where "
					+ this.idColumn + " = '" + id + "'");
	 ...
	 */
	@Override
	public T  queryOne(String colun,String publisheID) {
		synchronized (lock) {
			String selection =colun +"  = ?";
			String[] selectionArgs = { publisheID };
			KLog.d(AbDBDaoImpl.class, "[queryOne]: select * from " + this.tableName + " where "
					+ colun+" = '" + publisheID + "'");
			List<T> list = queryList(null, selection, selectionArgs, null, null, null,
					null);
	...
	public List<T> rawQuery(String sql, String[] selectionArgs,Class<T> clazz) {
		...
		try {
			lock.lock();
			checkDBOpened();
			KLog.d(AbDBDaoImpl.class, "[rawQuery]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			getListFromCursor(clazz,list, cursor);
		} catch (Exception e) {
			KLog.e(AbDBDaoImpl.class, "[rawQuery] from DB Exception.");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public boolean isExist(String sql, String[] selectionArgs) {
		...
		try {
			lock.lock();
			checkDBOpened();
			KLog.d(AbDBDaoImpl.class, "[isExist]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			if (cursor.getCount() > 0) {
				return true;
			}
		} catch (Exception e) {
			KLog.e(AbDBDaoImpl.class, "[isExist] from DB Exception.");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public List<T> queryList(String[] columns, String selection,
			...
			try {
				lock.lock();
				checkDBOpened();
				KLog.d(AbDBDaoImpl.class, "[queryList] from "+this.tableName+" where "+selection+"("+selectionArgs+")"+" group by "+groupBy+" having "+having+" order by "+orderBy+" limit "+limit);
					...
					for(T entity:list){
							...
							if(RelationsType.one2one.equals(type)){
								...
								if(listEntityClazz==null){
									KLog.e(AbDBDaoImpl.class, "对象模型需要设置List的泛型");
									return null;
								}
				...
				KLog.e(AbDBDaoImpl.class, "[queryList] from DB Exception");
				e.printStackTrace();
			} finally {
				closeCursor(cursor);
				lock.unlock();
	...
	public long insert(T entity, boolean flag) {
			...
			try {
				...
				KLog.d(AbDBDaoImpl.class, "[insert]: insert into " + this.tableName + " " + sql);
				...
				for (Field relationsField : allFields) {
					...
					if(RelationsType.one2one.equals(type)){
						...
						if(relationsEntity != null){
							...
							KLog.d(AbDBDaoImpl.class, "[insert]: insert into " + relationsTableName + " " + sql);
				...
				KLog.d(AbDBDaoImpl.class, "[insert] into DB Exception.");
				e.printStackTrace();
			}finally {
				lock.unlock();
			}
	...
	public long[] insertList(List<T> entityList,boolean flag) {
			...
			try {
				...
				for(int i=0;i<entityList.size();i++){
					...
					KLog.d(AbDBDaoImpl.class, "[insertList]: insert into " + this.tableName + " " + sql);
					...
					if(RelationsType.one2one.equals(type)){
						...
						if(relationsEntity != null){
							...
							KLog.d(AbDBDaoImpl.class, "[insertList]: insert into " + relationsTableName + " " + sql);
							db.insert(relationsTableName, null, relationsCv);
						}
				...
				KLog.d(AbDBDaoImpl.class, "[insertList] into DB Exception.");
				e.printStackTrace();
			} finally {
				lock.unlock();
			}
	...
	public int delete(int id) {
		...
		try {
			...
			KLog.d(AbDBDaoImpl.class, "[delete]: delelte from " + this.tableName + " where "
					+ where.replace("?", String.valueOf(id)));
			rows =  db.delete(this.tableName, where, whereValue);
		} catch (Exception e) {
			KLog.d(AbDBDaoImpl.class, "[delete] DB Exception.");
			e.printStackTrace();
		}finally{
			lock.unlock();
	    }
	...
	public int delete(String whereClause, String[] whereArgs) {
		...
		try {
			...
			KLog.d(AbDBDaoImpl.class, "[delete]: delete from " + this.tableName + mLogSql);
	...
	public int deleteAll() {
		...
		try {
			lock.lock();
			checkDBOpened();
			KLog.d(AbDBDaoImpl.class, "[delete]: delete from " + this.tableName );
	...
	public int update(T entity) {
		...
		try {
			...
			KLog.d(AbDBDaoImpl.class, "[update]: update " + this.tableName + " set " + sql
					+ " where " + where.replace("?", String.valueOf(id)));

			String[] whereValue = { Integer.toString(id) };
			rows = db.update(this.tableName, cv, where, whereValue);
		} catch (Exception e) {
			KLog.d(AbDBDaoImpl.class, "[update] DB Exception.");
			e.printStackTrace();
		} finally {
			lock.unlock();
		}
	...
	public List<Map<String, String>> queryMapList(String sql,String[] selectionArgs) {
		...
		try {
			lock.lock();
			checkDBOpened();
			KLog.d(AbDBDaoImpl.class, "[queryMapList]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			while (cursor.moveToNext()) {
			...
			KLog.e(AbDBDaoImpl.class, "[queryMapList] from DB exception");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public int queryCount(String sql, String[] selectionArgs) {
       ...
       try{
    	   lock.lock();
    	   checkDBOpened();
    	   KLog.d(AbDBDaoImpl.class, "[queryCount]: " + getLogSql(sql, selectionArgs));
           cursor = db.query(this.tableName, null, sql, selectionArgs, null, null,null);
           if(cursor != null){
        	   count = cursor.getCount();
           }
       }catch (Exception e){
    	   KLog.e(AbDBDaoImpl.class, "[queryCount] from DB exception");
           e.printStackTrace();
       }finally{
    	   closeCursor(cursor);
    	   lock.unlock();
	...
	public void execSql(String sql, Object[] selectionArgs) {
		try {
			lock.lock();
			checkDBOpened();
			KLog.d(AbDBDaoImpl.class, "[execSql]: " + getLogSql(sql, selectionArgs));
			...
			KLog.e(AbDBDaoImpl.class, "[execSql] DB exception.");
			e.printStackTrace();
		} finally {
			lock.unlock();
		}
