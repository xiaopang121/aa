package com.common.android.flog;
...
import com.common.android.futils.FileUtils;
...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	{
		String  dir="/sdcard"+"/"+mContext.getPackageName().toString()+"/log/";
		return dir;
	}
	...
	public boolean handleException(Throwable ex) {
		...
		new Thread() {
			public void run() {
				...
				//MobclickAgent.reportError(mContext, crashReport);
				System.exit(0);
				Looper.loop();
			}
	...
	private File save2File(String crashReport) {
		...
		if (Environment.getExternalStorageState().equals(
			...
			try {
				...
				if (!dir.exists())
					FileUtils.createDir(getLogDirectory());
				...
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						try {
							...
							String[] tos = {context.getResources().getString(R.string.receiver_error_report_email)};
							intent.putExtra(Intent.EXTRA_EMAIL, tos);

							intent.putExtra(Intent.EXTRA_SUBJECT,
									context.getResources().getString(R.string.report_subject));
							if (file != null) {
	...
	private PackageInfo getPackageInfo(Context context) {
		...
		try {
			...
			KLog.i("getPackageInfo err = " + e.getMessage());
		}
		if (info == null)
